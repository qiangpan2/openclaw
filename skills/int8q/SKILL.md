...
tmux new-session -d -s int8q-<short-name> \
  "VENV={baseDir}/.venv && (test -d \$VENV || uv venv --system-site-packages \$VENV) && uv pip install -p \$VENV --upgrade llmcompressor datasets sentencepiece && \$VENV/bin/python {baseDir}/scripts/quantize.py <model-path> [flags]; exec bash"
```
...
tmux new-session -d -s int8q-qwen3.5-0.8b \
  "VENV={baseDir}/.venv && (test -d \$VENV || uv venv --system-site-packages \$VENV) && uv pip install -p \$VENV --upgrade llmcompressor datasets sentencepiece && \$VENV/bin/python {baseDir}/scripts/quantize.py /workspace/models/Qwen/Qwen3.5-0.8B; exec bash"
```
...
tmux new-session -d -s int8q-llama3-8b \
  "VENV={baseDir}/.venv && (test -d \$VENV || uv venv --system-site-packages \$VENV) && uv pip install -p \$VENV --upgrade llmcompressor datasets sentencepiece && \$VENV/bin/python {baseDir}/scripts/quantize.py /workspace/models/meta-llama/Llama-3-8B --scheme W8A16; exec bash"
```
...
tmux new-session -d -s int8q-qwen3-32b \
  "VENV={baseDir}/.venv && (test -d \$VENV || uv venv --system-site-packages \$VENV) && uv pip install -p \$VENV --upgrade llmcompressor datasets sentencepiece && \$VENV/bin/python {baseDir}/scripts/quantize.py /workspace/models/Qwen/Qwen3-32B --samples 256 --smoothing 0.7; exec bash"
```
...
- Dependencies (llmcompressor, transformers, datasets, sentencepiece) are installed into a persistent venv at `{baseDir}/.venv/`, isolated from the system Python environment.
- The venv inherits system packages via `--system-site-packages`, so torch and CUDA libraries are reused without reinstallation.
